    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.cors(cors -> cors.configurationSource(corsConfigurationSource()))
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        //.requestMatchers("/api/auth/").permitAll() // Review if this is intended
                       .requestMatchers("/api/admin/employees/**").permitAll()// Secure admin endpoints
                       // .requestMatchers("/api/employee/**").hasRole("EMPLO // Example for employee endpoints
